...
class mysqliDs extends iDS
{
	...
	*/
	public function dsConnect()
	{
		...
		{
			function_exists('mysqli_connect') or ClsError::showError("DS00", "MYSQLI");
	...
	*/
	public function login($post, $chkpwd = true)
	{
		...
		$this->property["row"] = mysqli_fetch_array($this->property["result"]);
		$fail = 0;
		$failstop = 0;
	...
	*/
	public function changepasswd($post)
	{
		...
		return $this->property["conn"]->affected_rows;
	}
...
// 			$table = "`".str_replace(",","`,`", $this->property["dstable"])."`";
	...
	*/
	public function dsQueryInsertUpdateJoin()
	{
		...
		if ($insert) 
		{
			$this->property["inslast"] = $this->property["conn"]->insert_id;
	...
	*/
	public function dsGetRow($row = -1)
	{
		...
		if ($row >= 0) $this->property["result"]->data_seek($row) or ClsError::showError("DS004");
		switch($this->property["fetch"])
		{
			case "object":
				return $this->property["row"] = mysqli_fetch_object($this->property["result"]);	
			break;
			case "array":
				return $this->property["row"] = mysqli_fetch_array($this->property["result"]);	
			break;
			case "row":
				return $this->property["row"] = mysqli_fetch_row($this->property["result"]);	
			break;
			case "assoc":
				return $this->property["row"] = mysqli_fetch_assoc($this->property["result"]);	
			break;
		}
	}
	...
	*/
	public function dsCountRow()
	{
		...
		{
			if (!empty($this->property["result"]) && !is_bool($this->property["result"])) return $this->property["result"]->num_rows;
			return 0;
		}
	...
	*/  
    public function dsShowDatabases()
	{
		...
		$this->property['tot'] = $this->property["result"]->num_rows-1;
	}
   ...
   * @param string $database name of the database
	*/  
	public function dsShowTables($database )
	{
		  ...
		  $row = mysqli_fetch_array($this->property["result"]);
   ...
   * @param string $proc1 name of procedure
	...
	*/  
	public function dsShowColumnsResult($table = null)
	{
		...
		{
			$out[] = mysqli_fetch_field($this->property["result"], $i);
		}
		return $out;
	}
	...
	*/  
	public function dsShowForeignKey($db, $table)
	{
		...
		$result = array();
		while ($row = mysqli_fetch_array($this->property["result"]))
		{
			$rows = explode("\n", $row["Create Table"]);
	...
	*/
	public function dsImport($result, $method)
	{
		...
		foreach ($result as $row) 
		{
			...
			if ($method!="error") $conn->query($qry);
			else $conn->query($qry) or $this->ErrorDS003($qry);
			$this->property["inslast"] = $conn->insert_id;
			if (($this->property["inslast"] == 0) && ($this->property["dssavetype"] == "row")) ClsError::showError("DS007");
	 		if ($this->property["debug"]=="true") $system->debug($this->property["id"], $qry);
		}
  	}
